...
public class CommandLogin implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label,
        ...
        if (loggedIn) {
            String alreadyLoggedIn = utils.getStringFromYml("already_logged_in");
            player.sendMessage(alreadyLoggedIn);
            return true;
        }
        Boolean registered = authManager.isRegistered(playerUUID);
        if (!registered) {
            String notRegistered = utils.getStringFromYml("not_registered");
        ...
        if (!passwordProvided) {
            String passwordNotProvided = utils.getStringFromYml("password_not_provided");
